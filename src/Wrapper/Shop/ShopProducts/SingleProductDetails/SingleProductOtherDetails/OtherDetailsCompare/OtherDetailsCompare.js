...
const OtherDetailsCompare = ({ data, similarProducts }) => {
    ...
    if (products?.length === 0) {
        content = (
            <div className="h-[100px] w-full flex justify-center items-center text-[18px] text-gray-400 font-outfit">No Similer product found!</div>
        );
    } else {
        content = (
    ...
    return (
        <div className="mt-12">
            <h1 className="text-[#272727] text-3xl leading-[34px] font-semibold mb-10">Similar Products</h1>
            {content}
        </div>
    );
};
