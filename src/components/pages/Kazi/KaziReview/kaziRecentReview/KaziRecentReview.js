...
// Components
import KaziBestReviewsCard from "../../card/KaziBestReviewCard";
import KaziBestTwoReviewsCard from "../../card/KaziBestTwoReviewCard";

const KaziRecentReview = ({ reviewsData }) => {
            ...
            <div className="lg:max-w-[1000px] xl:max-w-[1200px] mx-auto py-20 mb-24">
                ...
                <div className="grid grid-cols-2 gap-8 mb-20">
                    {bestReviews.slice(0, 2).map(review => (
                        <KaziBestTwoReviewsCard key={review.id} {...{ review }} />
                    ))}
                </div>
                {/* Best reviews */}
                <div className="grid grid-cols-3 gap-8 px-28">
                    {topReviews.slice(0, 6).map(review => (
                        <KaziBestReviewsCard key={review.id} {...{ review }} />
                    ))}
                </div>
            </div>
        </div>
...
export default KaziRecentReview;
