...
// Components
import KaziPendingOrderCard from "../../card/KaziPendingOrderCard";

const PendingOrderForKazi = () => {
        ...
        <div className="mb-12 md:mb-16 lg:mb-28">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 lg:gap-12">
                {pendingOrdersInfo.map(order => (
                    <KaziPendingOrderCard key={order.id} {...{ order }} />
                ))}
            </div>
        </div>
    );
...
export default PendingOrderForKazi;
