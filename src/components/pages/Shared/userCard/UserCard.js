...
export const UserCard = ({ profile }) => {
    ...
    const profilePhoto = profile?.profilePhoto || "https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_1280.png";
    ...
    if (aboutYou?.length < 102) {
        ...
        intro = <div className=" h-[100px] flex items-center justify-center text-gray-400">No Intro Added</div>;
    }
                    ...
                    <div className="h-[350px]  relative group overflow-hidden Suggested-card mx-3">
                        ...
                        <div className="absolute h-[100px] w-full top-[200px] right-0 inset-0 bg-white opacity-90 text-black px-4 py-2 font-serif group-hover:top-[370px] transition ease-out duration-500 rounded-bl-[12px] rounded-br-[12px]">
                            <div className="flex gap-x-4 items-center">
                                ...
                                <p className="text-[17px] ml-[40px] text-black">{ageCalculator(profile?.dateOfBirth ? profile?.dateOfBirth : 0)}</p>
                            </div>
