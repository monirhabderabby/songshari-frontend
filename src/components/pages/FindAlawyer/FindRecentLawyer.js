...
export const FindRecentLawyer = () => {
    // Hook variable declaration
    const [page, setPage] = useState(1);

    const searchTerm = useSelector(state => state?.persistedReducer?.findLawyerKaziAgent?.lawyer?.searchTerm);
    const { data, isLoading, error } = useGetRecentMembersQuery({ searchTerm: searchTerm, page: page, role: "lawyer", limit: 8 });
