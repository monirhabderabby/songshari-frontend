...
const SinglePostHeader = ({ post }) => {
    ...
    return (
        <>
            ...
            <div className="">
                <p className="text-[#333333] my-6 ml-2 mr-[6px] text-justify">{post?.postBody}</p>
                {post?.attachment?.length > 0 && <img className="mt-[23px] max-h-[390px] w-full" src={post?.attachment[0]} alt="" />}
            </div>
        </>
    );
};
