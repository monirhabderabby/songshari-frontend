...
export const DynamicServicesContainer = ({ responsive }) => {
  const { id } = useParams();
  ...
  if (isLoading) {
    ...
    content = (
      <div className="h-[400px] w-full flex flex-col justify-center items-center">
        <FaServicestack />
        <span className="text-[16px] text-gray-400 font-Inter font-normal">
          No service released
        </span>
      </div>
    );
    ...
    content = (
      <div className="flex flex-col ">
        {services?.map((service, index) => {
          return (
            <DynamicServicesCard key={index} {...{ service, responsive }} />
          );
        })}
      </div>
    );
