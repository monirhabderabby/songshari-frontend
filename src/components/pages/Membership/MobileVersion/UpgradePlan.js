...
const UpgradePlan = () => {
  ...
  return (
    <div className="bg-[#F8F8FF]">
      ...
      <h1 className="text-center font-Inter font-medium text-sm text-[#000000] mt-2 -ml-3">
        Marvis Ighedosa
      </h1>
      ...
      <div className="plan-card flex mx-6 bg-[#FFFFFF] rounded font-Inter mt-6 mb-10 px-5 py-3">
        <div className="w-full">
          ...
          <h1 className="font-normal text-sm text-[#979C9E]">
            Upgrade for more features
          </h1>
        </div>
        <div className="flex items-center">
          <Link to={"/mobileFreePlan"}>
            <button className="font-medium text-[8px] text-[#FFFFFF] bg-red-400 rounded-[5px] bg-gradient-to-br from-[#E41272] to-[#4844A5] h-6 w-16">
              View Details
            </button>
          </Link>
        </div>
      ...
      <div className="flex flex-col gap-6 mt-6 mx-6 mb-10">
        {!isLoading &&
          ...
          data?.data?.plans?.map((item) => (
            <Link key={item._id} to={`/mobile/plan?plan=${item.title}`}>
