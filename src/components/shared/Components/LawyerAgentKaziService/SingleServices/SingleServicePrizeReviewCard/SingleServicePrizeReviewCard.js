...
const SingleServicePrizeReviewCard = ({
  price,
  deadline,
  _id,
  isActive,
  ...
  const [serviceDelete, { isSuccess: deleteSuccess, isLoading }] =
    useServiceDeleteMutation();
  const [changeServiceStatus, { isLoading: statusLoading }] =
    useChangeServiceStatusMutation();
  ...
  if (role.includes("agent")) {
    responsive
      ? (redirectPath = `/serviceEditMov/${_id}`)
