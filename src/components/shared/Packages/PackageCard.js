...
const PackageCard = ({ pack, packages, index, user, setSuccessSnackBarOpen, setSelectedPack, useCase }) => {
                ...
                <div className="text-left w-[340px]">
                    <div className={`rounded-lg bg-white p-6 shadow-xl`}>
                        <div className="relative">
                            <p
                                className={`text-[20px] font-semibold font-Inter mb-4 text-white w-48 py-[6px] pl-7 -ml-10 ${bgColor}`}
                                style={{
                                    clipPath: "polygon(0 0, 100% 0, 85% 50%, 100% 100%, 0 100%, 0% 50%)",
                                }}
                            >
                                {pack?.title}
                            </p>
                        ...
                        <div className="flex flex-col mb-4">
                            <h3 className="text-base font-Inter text-gray-600">
                                <span className="text-6xl font-semibold font-playFair">{pack?.priceMonth}</span>
                                BDT/month
                            </h3>
                            <p className="ml-6 text-2xl font-bold">Or,</p>
                            <p>
                                <span className="text-yellow-500 font-bold text-3xl">{pack?.priceMonthInPoint}</span> point/month
                            </p>
                        </div>
                        ...
                        {!pack?.title?.toLowerCase()?.includes("free") && (
                            <button onClick={handleBuyPack} className={`mt-6 rounded ${bgColor} px-4 py-2 font-medium text-white text-lg font-Inter`}>
                                Buy Pack
                            </button>
                        )}
                        {pack?.title?.toLowerCase()?.includes("free") && (
