...
export const IntroCard = ({ data, isLoading, error }) => {
  ...
  if (aboutYou?.length < 752) {
    ...
    intro = (
      <div className="h-[calc(356px-56px)] flex items-center justify-center text-gray-400">
        No Intro Added
      </div>
    );
  }
    ...
    intro = (
      <div className="grid grid-cols-1 gap-y-[11px]">
        {loadingLineNumber?.map((num) => (
          <div
            key={num}
            className="h-[11px] w-full bg-gray-200 animate-pulse rounded-[4px]"
          ></div>
        ))}
      </div>
    );
  ...
  if (
    !isLoading &&
    error &&
    error?.data?.message.includes("Cannot check authentication")
  ) {
    intro = (
