...
const UserCanceledServices = ({ responsive }) => {
    ...
    if (isLoading) {
        ...
        content = <div className="h-[150px] w-full flex justify-center items-center text-gray-400">You have no onGoing service</div>;
    } else if (!isLoading && orders.length > 0) {
        content = (
            <div className="flex flex-col items-center gap-y-[10px]">
                {orders?.map((order, i) => {
                    return <ServiceCardV2 key={order._id} service={order} status="cancelled" {...{ responsive }} i={i} />;
                })}
            </div>
        );
    }
