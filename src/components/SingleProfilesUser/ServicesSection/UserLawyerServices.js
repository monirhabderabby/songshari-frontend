...
const UserLawyerServices = () => {
    ...
    if (isLoading) {
        ...
        content = <div className="h-[150px] w-full flex justify-center items-center text-gray-400">No Lawyer service found</div>;
    } else if (!isLoading && services.length > 0) {
        content = (
            <div className="flex flex-col items-center gap-y-[10px]">
                {services?.map(service => {
                    return <ServiceCardV2 key={service._id} service={service} />;
                })}
            </div>
        );
    }
