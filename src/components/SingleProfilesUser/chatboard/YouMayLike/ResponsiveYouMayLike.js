...
import { ResponsiveCardYouMayLike } from "../../../shared/Cards/CardYouMayLike/ResponsiveCardYouMayLike";
import { YouMayLikeSkeletionLoader } from "../../../shared/Cards/Loader/YouMayLikeSkeletionLoader";

export const ResponsiveYouMayLike = () => {
    // varible declation
    // hook variables
        ...
        content = (
            <div className="flex flex-col gap-y-[19px]">
                {data?.data?.suggestion?.slice(0, 4).map(profile => {
                    return <ResponsiveCardYouMayLike key={profile._id} {...{ data: profile }} />;
                })}
            </div>
        );
    }
    ...
    return (
        <div>
            <div className="mt-[70px] ml-[120px] lg:hidden md:hidden">
            ...
            <div className="mt-[70px] ml-[500px] lg:hidden md:block hidden">
                <div className="flex justify-start items-center mx-1 gap-6 mb-6">
                    <BsCommand className="text-[#2E3A59] text-2xl" />
                    <h1 className="text-[#333333] text-2xl leading-[34px] font-bold">You May Likee</h1>
                </div>
                {/* Render Data from content variable */}
                <div>{content}</div>
            </div>

            <div className="mt-[70px] ml-[120px] hidden lg:hidden">
                <div className="flex justify-start items-center mx-1 gap-6 mb-6">
                    <BsCommand className="text-[#2E3A59] text-2xl" />
                    <h1 className="text-[#333333] text-2xl leading-[34px] font-bold">You May Like</h1>
                </div>
