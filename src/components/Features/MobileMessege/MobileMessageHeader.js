...
export const MobileMessageHeader = ({ profile }) => {
  ...
  // own functions
  const { generateRoomID, profilePhotoDecisionMaker } = customFunc;
      ...
      <div className=" pl-[30px] w-full h-full flex items-center justify-between max-w-[990px]">
        <div className="flex items-center h-full gap-x-[10px]">
          ...
          <div className="h-full flex flex-col justify-center">
            <h3 className="text-[#000000] font-bold font-Poppins text-[17px]">
              {profile?.firstName + " " + profile?.lastName}
            </h3>
            <p className="text-[rgba(0,0,0,0.35)] font-medium text-[13px] tracking-[-0.08px]">
              Active
            </p>
          </div>
        </div>
        <div className="flex items-center gap-x-[24px]">
          <FaPhoneAlt
            className="text-[#A32BCA] h-[24px] w-[24px]"
            onClick={callHandler}
          />
